...
<head>
    ...
    <script>
        ...
        function baseHandleAjaxData(routes, removeAll) {
            ...
            for (var j = 0; j < routes.routes.length; ++j) {
                ...
                for (var i = 0; i < routeData.block.length; ++i) {
                    ...
                    var tripId = blockData.trip.shortName != null ? blockData.trip.shortName : blockData.trip.id;
        ...
        function initializeLoadAllData(routes) {
            ...
            var routeNames = routes.routes.map(function (d) {
                return d.name
            })
...
<div id="summary">
    ...
    <span id="percentWithVehiclesLabel"
          title="Percentage of blocks that have an assigned and predictable vehicle"><fmt:message
            key="div.assigned"/></span>
    <span id="percentWithVehiclesValue"
          title="Percentage of blocks that have an assigned and predictable vehicle"></span>
    <span id="percentLateLabel"
          title="Percentage of blocks where vehicle is more than <%= ScheduleAdherenceController.getScheduleLateSeconds()/60 %> minutes late"><fmt:message
            key="div.clate"/>:</span>
    <span id="percentLateValue"
          title="Percentage of blocks where vehicle is more than <%= ScheduleAdherenceController.getScheduleLateSeconds()/60 %> minutes late"></span>
    ...
    <span id="percentEarlyLabel"
          title="Percentage of blocks where vehicle is more than <%= ScheduleAdherenceController.getScheduleEarlySeconds()/-60 %> minute(s) early"><fmt:message
            key="div.cearly"/>:</span>
    <span id="percentEarlyValue"
          title="Percentage of blocks where vehicle is more than <%= ScheduleAdherenceController.getScheduleEarlySeconds()/-60 %> minute(s) early"></span>
