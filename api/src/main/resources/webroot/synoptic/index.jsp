...
<script type="text/javascript">
    ...
    function vehicleUpdate(vehicleDetail, status) {
        ...
        for (var i in vehicleDetail.vehicles) {
            ...
            buses.push({
                id: vehicle.id,
                projection: vehicle.distanceAlongTrip / getShapeLength(vehicle.tripPattern),
                identifier: _identifier,
                direction: directionVehicle,
                gpsTimeStr: gpsTimeStr,
                nextStopName: vehicle.nextStopName,
                schAdhStr: vehicle.schAdhStr,
                trip: vehicle.trip,
                schAdh: vehicle.schAdh,
    ...
    function routeConfigCallback(routeDetail, status) {
        ...
        for (var i in routeDetail.routes[0].direction) {
            ...
            for (var j in routeDetail.routes[0].direction[i].stop) {
                ...
                stops.push({
                    ...
                    projection: projectionStop,
                    direction: directionStop,
                    distance: distanceOverPath
                });
            }
            if (i > 0)
                showReturn = true;
        ...
        var params = {
            ...
            infoStop: function (data) {
                ...
                return "<table class=\"table\"><th >" + data.identifier + "</th><tr><td>distance: " + parseFloat(data.distance).toFixed(2) + " m. </td></tr></table>"
            },
            ...
            infoVehicle: function (data) {
                console.log(data.identifier);
                return "<table class=\"table\"><th >" + data.identifier + "</th><tr><td>GPSTime: " + data.gpsTimeStr + " </td></tr><tr><td>NextStop: " + data.nextStopName + "</td></tr><tr><td>schAdh: " + data.schAdhStr + "</td></tr><tr><td>trip: " + data.trip + ((data.isScheduledService != undefined && data.isScheduledService == false) ? ("</td></tr><tr><td>Start Time: " + dateFormat(data.freqStartTime / 1000)) : "") + "</td></tr><tr><td>headway: " + ((data.headway == -1) ? "-" : ((data.headway / 60000).toFixed(2)) + " min.") + "</td><tr></table>"
            },
