...
private static final int MAX_NUM_DAYS = 7;
	...
	public static String getTripWithTravelTimes(String agencyId, String tripId,
		...
		String sql = "SELECT"
				...
				+ "	 THEN DATE('" + date + "') + trip_scheduledtimeslist.arrivaltime * interval '1 second'"
				+ "	 ELSE ADDeparture.scheduledTime"
				+ "	 END AS scheduledTime"
				+ "	,CASE WHEN ADDeparture.scheduledTime ISNULL"
				+ "	 THEN regexp_replace(CAST(DATE_TRUNC('second', DATE('" + date + "') + trip_scheduledtimeslist.arrivaltime * interval '1 second') - DATE_TRUNC('second', arrivalsdepartures.time::timestamp) AS VARCHAR), '^00:', '') "
