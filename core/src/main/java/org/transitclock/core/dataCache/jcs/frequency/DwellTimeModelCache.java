...
public class DwellTimeModelCache implements org.transitclock.core.dataCache.DwellTimeModelCacheInterface {
    ...
    private static IntegerConfigValue maxDwellTimeAllowedInModel = new IntegerConfigValue(
            "org.transitclock.core.dataCache.jcs.maxDwellTimeAllowedInModel",
            2 * Time.MS_PER_MIN,
            "Max dwell time to be considered in dwell RLS algotithm.");
    private static LongConfigValue maxHeadwayAllowedInModel = new LongConfigValue(
    ...
    public synchronized void addSample(ArrivalDeparture event, Headway headway, long dwellTime) {
        ...
        if (cache.get(key) != null) {
            ...
            if (rls.getRls() != null) {
                ...
                logger.debug(
                        "Predicted dwell after: " + prediction + " for: " + key + " with samples: " + rls.numSamples());
            }
        } else {
        ...
        for (IpcArrivalDeparture event : stopData) {
            if (event.isArrival()) {
                if (!event.getVehicleId().equals(arrival.getVehicleId())) {
                    ...
                    // If trip id the same check that not the same trip by checking the
                    // freqStartTime
                    if (event.getTripId().equals(arrival.getTripId())) {
                            ...
                            if (!event.getFreqStartTime().equals(arrival.getFreqStartTime())) {
                                if (event.getStopId().equals(arrival.getStopId())) {
                                    if (event.getTime().getTime()
                                            < arrival.getTime().getTime()) return event;
                                }
                            }
                        }
                    }
    ...
    public Long predictDwellTime(StopPathCacheKey cacheKey, Headway headway) {
        ...
        if (rls != null && rls.getRls() != null) {
            ...
            // If silly values returned then need to reset model and allow it use the super
            // prediction.
