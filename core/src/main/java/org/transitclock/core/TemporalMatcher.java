...
public class TemporalMatcher {
     ...
     *     first spatial match. This way if matching a vehicle to middle of trip then will more
     *     likely match to the middle instead of the layover at the end of the trip. This is
     *     important when starting up the system while vehicles are already running.
    ...
    private TemporalDifference determineHowFarOffScheduledTime(
        ...
        if (deltaFromSchedule.isWithinBoundsForInitialMatching()) {
            ...
            logger.debug(
                    "For vehicleId={} determineHowFarOffScheduledTime() " + "returning expectedTimeDelta={} for {}",
                    vehicleId,
                    deltaFromSchedule,
                    spatialMatch);
    ...
    /**
     * Returns whether the currentSpatialMatch is a better match than the bestTemporalMatchSoFar.
     * Uses differenceFromExpectedTime for the current match to determine how good the new match is
     * temporally.
     *
    ...
    private static boolean currentMatchIsBetter(
        ...
        if (differenceFromExpectedTime.betterThanOrEqualTo(bestTemporalMatchSoFar.getTemporalDifference())) {
            return true;
        }
     ...
     * temporally match to a match past the layover due to system thinking that vehicle first needs
     * to go back to the layover and then to the spatial match. This leads to an overly long
     * expected travel time and the result would be that the system thinks the vehicle is at the
     * match earlier than expected. At the same time it will think that it is late with respect to
     * the layover, but since late is considered a better match than early vehicle could be wrongly
     * temporally matched to layover again. As the vehicle moves further along the trip this only
     * gets worse since system will expect a longer and longer travel time for vehicle to go back to
     * layover and then to the spatial match. So could get incorrect layover match for quiet a
     * while, until the layover distance transitclock.core.layoverDistance, which can be large, is
     * exceeded.
     *
     ...
     * roam around), the previous match for vehicle is same layover as the current spatial match,
     * the distance from the layover stop is increasing, and there is a subsequent non-layover
     * spatial match that could be used. If all these conditions are true then the current layover
     ...
     * @return The match passed in that best matches temporally where vehicle should be. Returns
     *     null if no adequate temporal match.
     */
    public TemporalMatch getBestTemporalMatchComparedToSchedule(
        ...
        for (int i = 0; i < spatialMatches.size(); ++i) {
            ...
            TemporalDifference differenceFromExpectedTime = determineHowFarOffScheduledTime(
     ...
     *
     * <p>Don't want to look through all trips for a block because then might match to first block
     * the next day, which would be a problem. Instead need to only look at trips that are
     * considered to be valid. Therefore should only pass in upcoming trips as the potentialTrips
     * parameter.
     *
     * @param avlReport
