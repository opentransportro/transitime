...
public class SpatialMatcher {
     ...
     * the noise of GPS and also the flexibility of layovers a best spatial match might be
     * determined to be before the previous match. But that would be a problem when determining
     * arrivals/ departures and such. Therefore need to only search starting from previous match.
     *
    ...
    /**
     * Goes through entire TripPattern for specified Trip and determines spatial matches. Matches
     * must be within getMaxAllowableDistanceFromSegment() except layovers are always included since
     * vehicle are allowed to be away from the route path during layovers. First checks to see if
     * the avlReport location is near the extent of the trip pattern. If it is not then can save
    ...
    /**
     * For list of spatial matches passed in returns the first non-layover one. This is needed
     * because for a trip always get a layover match in addition to the possible real spatial
     * matches. But the layover match doesn't need heading to be correct.
     *
    ...
    /**
     * Checks to see if for a non-layover match if can verify that the vehicle is moving in the
     * proper direction. This is intended to avoid matching vehicle to wrong trip. Only concerned
     * with non-layover matches because for layovers heading doesn't matter since vehicle is
     * supposed to be able to move around on indeterminate path to the layover stop.
     *
     * <p>If cannot confirm that the heading of the vehicle matches then should reject all matches
     * so that won't wrongly match to a layover for another trip. When get another AVL report for
     * the vehicle will then be able to see if heading proper using two AVL reports.
     *
     * @param spatialMatch The spatial match that should be investigated
     * @param vehicleStatus For determining previous AVL reports
     * @param matchingType for keeping track of what kind of spatial matching being done
    ...
    public boolean problemMatchDueToLackOfHeadingInfo(
        ...
        SpatialMatch previousNonLayoverSpatialMatch = getFirstNonLayoverSpatialMatch(spatialMatchesForPreviousReport);
     ...
     * @param tripsToInvestigate List of trips that should bother investigating. The calling
     *     function can determine which trips are currently active and pass that list in such that
    ...
    public List<SpatialMatch> getSpatialMatches(
        ...
        for (Trip trip : tripsToInvestigate) {
            if (tripPatternIdsCovered.contains(trip.getTripPattern().getId())) {
                ...
                List<SpatialMatch> matchListForIteration = new ArrayList<SpatialMatch>(spatialMatchesForAllTrips);
                for (SpatialMatch spatialMatch : matchListForIteration) {
                    String spatialMatchTripPatternId =
                            spatialMatch.getTrip().getTripPattern().getId();
    ...
    /**
     * Goes through the Block assignment data and determines the closest spatial matches that are
     * not for layovers. For first matching a vehicle to a block assignment. Matches must be within
     * getMaxAllowableDistanceFromSegment() except layovers are always included since vehicle are
     * allowed to be away from the route path during layovers.
     *
     * @param avlReport The AVL report to match to the block
    ...
    public List<SpatialMatch> getSpatialMatchesForAutoAssigning(
        ...
        List<SpatialMatch> allSpatialMatches =
                getSpatialMatches(avlReport, block, tripsToInvestigate, MatchingType.AUTO_ASSIGNING_MATCHING);
     ...
     * distance for the route if it is set. If max distance for route is not set then uses the
     * global CoreConfig.getMaxDistanceFromSegment().
     *
     * @param route
     * @param matchingType for keeping track of what kind of spatial matching being done
     ...
     * last stop of the previous trip and the layover stop, or within the configured layover
     * distance.
     *
     * @param vehicleId
     * @param avlLoc
     ...
     * @return
     */
    private boolean withinAllowableDistanceOfLayover(String vehicleId, Location avlLoc, Indices potentialMatchIndices) {
     ...
     * getMaxAllowableDistanceFromSegment() except layovers are always included since vehicle are
     * allowed to be away from the route path during layovers. To be called for a series of
     * segments.
     *
    ...
    private void processPossiblePotentialMatch(
        ...
        if (atLayover
            ...
            logger.debug(
                    "For vehicleId={} segment is at a layover so adding " + "it to list of spatial matches. {}",
                    avlReport.getVehicleId(),
                    spatialMatch);
            spatialMatches.add(spatialMatch);
        }
     ...
     *     opposed to null)
     */
    public List<SpatialMatch> getSpatialMatches(VehicleStatus vehicleStatus) {
        // Some convenience variables
        TemporalMatch previousMatch = vehicleStatus.getMatch();
        ...
        if (previousMatch
                        ...
                        .getMaxSpeed()
                != null) {
            timeBetweenFixesMsec = vehicleStatus.getAvlReport().getTime()
                    - vehicleStatus.getPreviousAvlReportFromSuccessfulMatch().getTime();
            distanceAlongPathToSearch = previousMatch
                            .getTrip()
                            .getStopPath(previousMatch.getIndices().getStopPathIndex())
                            .getMaxSpeed()
                    * timeBetweenFixesMsec
                    / Time.MS_PER_SEC;
            logger.info(
                    "Using alternate max speed {} for vehicle {} on stop path index {} which"
                            + " results in a distance along segment to search of {}.",
                    previousMatch
                            .getTrip()
                            .getStopPath(previousMatch.getIndices().getStopPathIndex())
                            .getMaxSpeed(),
        ...
        while (!indices.pastEndOfBlock(vehicleStatus.getAvlReport().getTime(), dbConfig)
            ...
            // For next iteration through while loop
            indices.increment(vehicleStatus.getAvlReport().getTime(), dbConfig);
        }
        ...
        if (!block.isNoSchedule()
            ...
            SpatialMatch matchAtEndOfBlock = new SpatialMatch(
                    vehicleStatus.getAvlReport().getTime(),
                    block,
