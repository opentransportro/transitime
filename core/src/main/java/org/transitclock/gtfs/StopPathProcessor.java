...
public class StopPathProcessor {
     ...
     *     the stopPaths are offset to the left. If 0.0 then of course the stopPaths are not offset
     *     at all.
    ...
    /**
     * Offsets the stopPaths to the right the distance specified. Useful for when shape data is
     * street center line. By offsetting the data the stopPaths in the different directions won't
     * overlap on the map when zoomed in adequately. Also, makes debugging easier since won't have
     * confusing overlapping stopPaths.
     *
    ...
    private List<Location> getOffsetLocations(List<GtfsShape> gtfsShapes) {
        ...
        Location beginningOffsetLoc =
                Geo.rightOffsetBeginningLoc(gtfsShape0.getLocation(), gtfsShape1.getLocation(), offsetDistance);
        ...
        for (int i = 0; i < gtfsShapes.size() - 2; ++i) {
            ...
            Location offsetVertex =
                    Geo.rightOffsetVertex(g1.getLocation(), g2.getLocation(), g3.getLocation(), offsetDistance);
    ...
    private BestMatch determineBestMatch(
        ...
        double distanceBetweenStopsAsCrowFlies =
                previousStop == null ? Double.MAX_VALUE : (new Vector(previousStop.getLoc(), stop.getLoc())).length();
        ...
        for (int shapeIndex = previousShapeIndex; shapeIndex < shapeLocs.size() - 1; ++shapeIndex) {
            ...
            if (stopToShapeDistance < bestStopToShapeDistance - 0.0001) {
                ...
                boolean specialLoopBackToBeginningCase = bestMatch != null
                        && stopIndex == 0
     ...
     *     StopPath objects.
     */
    private void determinePathSegmentsMatchingStopsToShapes(List<Location> shapeLocs, TripPattern tripPattern) {
        ...
        for (int stopIndex = 0; stopIndex < tripPattern.getStopPaths().size(); ++stopIndex) {
            ...
            BestMatch bestMatch = determineBestMatch(
                    tripPattern,
                    stopIndex,
                    previousShapeIndex,
            ...
            if (previousLocation == null && bestMatch.stopToShapeDistance > maxStopToPathDistance) {
                ...
                if (bestMatch.shapeIndex != previousShapeIndex) {
                    ...
                    for (int shapeIndex = previousShapeIndex + 1; shapeIndex <= bestMatch.shapeIndex; ++shapeIndex) {
    ...
    /**
     * Goes through the locations for the StopPath and filters them to make sure that they are not
     * too short. Don't want lots of tiny segments for curves cluttering up the database and mucking
     * travel times more complicated.
     *
     * @param locations the locations that are to be filtered
     * @param maxDistanceForEliminatingVertices If a vertex is off by less this distance from the
     *     line connecting the previous and next vertex then this one can be filtered out.
     */
    private void filterShortSegments(List<Location> locations, double maxDistanceForEliminatingVertices) {
    ...
    /**
     * Determines the path segments for each trip pattern. If shapes.txt GTFS file has a shape for
     * the trip pattern then that data is used. Otherwise will simply connect the stops with
     * straight line stopPaths.
     *
     ...
     * centerline data. By offsetting the path segments for the different directions won't
     * completely overlap on the map. Then the stops are matched to the shapes in order to create
     * the Paths. Each path is for a separate stop and the path ends at the stop. The path segments
     * are also filtered so that segments that are too short or too long are adjusted.
     */
    public void processPathSegments() {
        ...
        // Let user know what is going on
        logger.info("Finished processing and filtering path segment data. " + "Took {} msec.", timer.elapsedMsec());
    }
}
