...
public class BlocksProcessor {
     ...
     * direction but have no schedule time associated. Useful for special situations such as
     * baseball game events where transit agency needs to add additional unscheduled service.
     *
    ...
    private void addUnscheduledBlocks(int configRev, List<Block> blocks, Set<String> serviceIdsUsed) {
        ...
        for (GtfsRoute gtfsRoute : gtfsRoutes) {
            if (gtfsRoute.shouldCreateUnscheduledBlock()) {
                ...
                String blockId =
                        (gtfsRoute.getRouteShortName() != null ? gtfsRoute.getRouteShortName() : gtfsRoute.getRouteId())
                ...
                if (longestTripPatternForDirection0 != null && longestTripPatternForDirection1 != null) {
                    ...
                    for (String serviceId : serviceIdsUsed) {
                        Block block = new Block(
                                configRev, blockId, serviceId, startTimeForBlock, endTimeForBlock, tripsListForBlock);
    ...
    public List<Block> process(int configRev) {
        ...
        for (String serviceId : tripListByBlocksByServiceMap.keySet()) {
            ...
            for (String blockId : tripListForBlocksMap.keySet()) {
                ...
                // Create the Block
                Block block =
                        new Block(configRev, blockId, serviceId, startTimeForBlock, endTimeForBlock, tripsListForBlock);
