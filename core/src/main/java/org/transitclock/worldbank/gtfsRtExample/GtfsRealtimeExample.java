...
public class GtfsRealtimeExample {
     ...
     */
    private static VehiclePosition createVehiclePosition(VehicleData vehicleData) throws ParseException {
        // Create the VehicleDescriptor information
        VehicleDescriptor.Builder vehicle = VehicleDescriptor.newBuilder().setId(vehicleData.getVehicleId());
        ...
        if (vehicleData.getHeading() != null) position.setBearing(Float.parseFloat(vehicleData.getHeading()));
    ...
    private static FeedMessage createMessage(List<VehicleData> vehicleDataList) {
        ...
        FeedHeader.Builder feedheader =
                FeedHeader.newBuilder().setGtfsRealtimeVersion("1.0").setTimestamp(System.currentTimeMillis());
        message.setHeader(feedheader);

        for (VehicleData vehicleData : vehicleDataList) {
            FeedEntity.Builder vehiclePositionEntity = FeedEntity.newBuilder().setId(vehicleData.getVehicleId());
     ...
     *
     * <p>This works for 16-bit characters only. It does not handle 32-bit characters encoded with
     * the contortionist use of the low (0xdc00..0xdfff) and high(0xd800..0xdbff) bands of surrogate
     * characters.
     *
        ...
        do {
            ...
            if (outputHumanReadable) {
                ...
                System.out.println("Writing GTFS-realtime messsage to file \""
                        ...
                        + "\" ...");
                if (usingPolling()) overwriteMessageToFile(message, ConfigurableParameters.OUTPUT_FILE_NAME);
