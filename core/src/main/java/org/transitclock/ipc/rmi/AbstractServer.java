...
public abstract class AbstractServer {
     ...
     * It also periodically rebinds the object every 30 seconds so that if the rmiregistry gets
     * restarted the RMI object will then automatically become available again. Sends an e-mail
     * alert when rebinding if the rmiregistry is not available.
     *
     * <p>Protected because should only be instantiated by a superclass factory.
     *
     * <p>This constructor "leaks" a reference to the object before the constructor has completed
     * via UnicastRemoteObject.exportObject(). This normally should not be done because the function
     * could cause the object to be used before it has been fully constructed. But by leaking the
     * object we can guarantee that any Server class that inherits from this class will be properly
     * initialized when the subclass constructor is called. This is fine as long as the Server
     * object constructor doesn't do any further initialization.
     *
