...
public class DbAvlReader extends Module {
    ...
    private static StringConfigValue dbQuery = new StringConfigValue(
            "transitclock.avl.dbQuery",
            "select ID as Id, BusID as busId,  CAST(CONCAT(RptDate, ' ', RptTime) AS"
                    + " DATETIME) AS rptDateTime, LatDD as lat, LonDD as lon, LogonRoute as"
                    + " logonRoute, LogonTrip as logonTrip, RptDate as reportDate from"
                    + " tblbuses;",
    ...
    private static IntegerConfigValue pollingRateMsec = new IntegerConfigValue(
