...
public class AvlSqsClientModule extends Module {
    ...
    private static IntegerConfigValue messageLogFrequency = new IntegerConfigValue(
            "transitclock.avl.messageLogFrequency",
            DEFAULT_MESSAGE_LOG_FREQUENCY,
            "How often (in count of message) a log message is output " + "confirming messages have been received");
    ...
    private static ClassConfigValue unmarshallerConfig = new ClassConfigValue(
    ...
    public AvlSqsClientModule(String agencyId) throws Exception {
        ...
        logger.info(
                "Starting AvlClient for agencyId={} with " + "maxAVLQueueSize={}, numberThreads={} and url={}",
                agencyId,
                maxAVLQueueSize,
                numberThreads,
                _url);
        ...
        _messageUnmarshaller =
                (SqsMessageUnmarshaller) unmarshallerConfig.getValue().newInstance();
    ...
    private class StatusTask implements Runnable {
        ...
        public void run() {
            while (!Thread.interrupted()) {
                try {
                    long lastAvlReportTime = (System.currentTimeMillis()
                                    - AvlProcessor.getInstance().lastAvlReportTime())
                            / 1000;
                    logger.info(
                            "Queue Size Report:  AVL last report {}s, recieve={}, deserialize={},"
                                    + " ack={}, archive={}",
                            lastAvlReportTime,
                            _receiveQueue.size(),
                            _deserializeQueue.size(),
