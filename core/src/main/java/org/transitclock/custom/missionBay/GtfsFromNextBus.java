...
public class GtfsFromNextBus {
    ...
    /******************* Parameters *************************/
    private static StringConfigValue agencyId = new StringConfigValue(
            "transitclock.gtfs.agencyId", "missionBay", "Agency name used in resulting GTFS files.");

    private static StringConfigValue nextBusAgencyId = new StringConfigValue(
            "transitclock.gtfs.nextBusAgencyId", "sf-mission-bay", "Agency name that NextBus uses.");

    private static StringConfigValue nextBusFeedUrl = new StringConfigValue(
            "transitclock.gtfs.nextbusFeedUrl",
            "http://webservices.nextbus.com/service/publicXMLFeed",
            "The URL of the NextBus feed to use.");

    private static StringConfigValue gtfsDirectory = new StringConfigValue(
            "transitclock.gtfs.gtfsDirectory", "C:/GTFS/", "Directory where resulting GTFS files are to be written.");

    private static StringConfigValue gtfsRouteType = new StringConfigValue(
            "transitclock.gtfs.gtfsRouteType", "3", "GTFS definition of the route type. 1=subway 2=rail " + "3=buses.");

    private static StringConfigValue serviceId = new StringConfigValue(
            "transitclock.gtfs.serviceId",
            "wkd",
            "The service_id to use for the trips.txt file. Currently " + "only can handle a single service ID.");

    private static StringListConfigValue validBlockIds = new StringListConfigValue(
    ...
    private static String getNextBusUrl(String command, String options) {
        return nextBusFeedUrl.getValue() + "?command=" + command + "&a=" + nextBusAgencyId.getValue() + "&" + options;
    }
     ...
     */
    private static Document getNextBusApiInputStream(String command, String options) throws IOException, JDOMException {
    ...
    /**
     * Processes XML data for route and extracts stop information. Adds the resulting GTFS stop to
     * the gtfsStopsMap.
     *
    ...
    /**
     * For each route gets routeConfig input stream from NextBus API and creates and processes an
     * XML Document object to read the data for the stops. Then writes the stops.txt GTFS file.
     *
     * @param routeIds
     */
    ...
    private static GtfsRoute getGtfsRoute(Document doc) {
        ...
        GtfsRoute gtfsRoute = new GtfsRoute(
                routeId, agencyId.getValue(), null, title, gtfsRouteType.getValue(), color, oppositeColor);
        return gtfsRoute;
    }
     ...
     * something shorter, such as just the blockId and time, but it is nice to see the full context
     * of the trip in the trips.txt file.
     *
     * @param routeId
     * @param blockId
     * @param timeStr
     ...
     * matches uses the direction with the fewest stops so that can handle case where a schedule
     * trip could match both a direction and a direction that contains additional stops that are not
     * in the schedule.
     *
     * @param stopIdsInSchedForTrip So can determine if trip goes to calt4th stop
     * @param directionsForRoute
     * @param routeId
     * @param tripStartTimeStr Needed so can differentiate between morning and afternoon trips,
     *     which an be different for Mission Bay west route.
     * @return
     */
    private static Dir determineDirection(
            List<String> stopIdsInSchedForTrip, List<Dir> directionsForRoute, String routeId, String tripStartTimeStr) {
        //		Dir bestDir = null;
        //
    ...
    private static void writeGtfsTrip(
        ...
        GtfsTrip gtfsTrip = new GtfsTrip(
                routeId,
                serviceId.getValue(),
                tripId,
                tripHeadsign,
    ...
    private static boolean firstStopInNextTrip(
            int i, String routeId, int beginIdx, List<StopTime> stopTimesForBlock, List<Dir> dirsForRoute) {
    ...
    private static void processTripsAndStopTimesForRoute(
            GtfsTripsWriter tripsWriter, GtfsStopTimesWriter stopTimesWriter, String routeId, Document scheduleDoc) {
