...
public class SfmtaApiCaller {
    ...
    private static StringConfigValue telemetryUrl = new StringConfigValue(
            "transitclock.sfmta.telemetryUrl",
            "https://services.sfmta.com/shuttle/api/Telemetries/",
            "The telemetry URL for the SFMTA API");

    private static StringConfigValue stopsUrl = new StringConfigValue(
            "transitclock.sfmta.telemetryUrl",
            "https://services.sfmta.com/shuttle/api/StopEvents/",
            "The stops URL for the SFMTA API");

    private static StringConfigValue login =
            new StringConfigValue("transitclock.sfmta.login", "TransitTimeApiUser", "Login for basic authentication.");

    private static StringConfigValue password = new StringConfigValue(
    ...
    private static StringConfigValue techProviderId =
            new StringConfigValue("transitclock.sfmta.techProviderId", "248", "The TechProviderId for the SFMTA API");

    private static StringConfigValue shuttleCompanyId = new StringConfigValue(
            "transitclock.sfmta.shuttleCompanyId", "11", "The ShuttleCompanyId for the SFMTA API");

    private static IntegerConfigValue timeout = new IntegerConfigValue(
            "transitclock.sfmta.timeout",
            10000,
            "Timeout in msec for API calls. A timeout of 0 is " + "interpreted as an infinit timeout.");

    private static IntegerConfigValue avlReportsBatchSize = new IntegerConfigValue(
    ...
    private static class StopInfo {
         ...
         */
        public StopInfo(
                String vehicleId, int stopId, double stopLat, double stopLon, Date arrivalTime, Date departureTime) {
    ...
    private static void postAvlReports(List<AvlReport> avlReports) {
        ...
        if (post(telemetryUrl.getValue(), jsonStr)) {
            logger.info("Successfully pushed out {} telemetry data points to " + "SFMTA API", avlReports.size());
    ...
    public static void main(String args[]) {
        AvlReport avlReport1 = new AvlReport("693", System.currentTimeMillis(), 37.12345, -122.4567, null);
        AvlReport avlReport2 = new AvlReport("694", System.currentTimeMillis(), 37.12345, -122.4567, null);
