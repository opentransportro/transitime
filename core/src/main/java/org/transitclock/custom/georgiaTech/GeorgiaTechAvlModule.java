...
public class GeorgiaTechAvlModule extends AvlModule {
    ...
    private static StringConfigValue georgiaTechFeedDomainName = new StringConfigValue(
            "transitclock.avl.georgiaTechFeedDomainName",
            "The domain name for the socket connection for the " + "Georgia Tech AVLfeed.");

    private static IntegerConfigValue georgiaTechFeedPort = new IntegerConfigValue(
    ...
    private void readVehicleDataFromSocket(BufferedReader reader) throws IOException {
        ...
        AvlReport avlReport = new AvlReport(
                vehicleId, gpsLoc.getTime(), gpsLoc.getLocation(), gpsLoc.getSpeed(), gpsLoc.getHeading(), "GeorgiaT");
    ...
    /**
     * Reads data from socket forever and processes it. Catches exceptions so really will run
     * forever.
     */
