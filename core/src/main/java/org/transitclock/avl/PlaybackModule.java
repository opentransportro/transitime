...
public class PlaybackModule extends Module {
    ...
    protected static StringConfigValue playbackStartTimeStr = new StringConfigValue(
    ...
    protected static StringConfigValue playbackEndTimeStr =
            new StringConfigValue("transitclock.avl.playbackEndTime", "", "Date and time of when to end the playback.");

    protected static BooleanConfigValue playbackRealtime = new BooleanConfigValue(
            "transitclock.avl.playbackRealtime",
            false,
            "Playback at normal time speed rather than as fast as possible.");

    protected static IntegerConfigValue playbackSkipIntervalMinutes = new IntegerConfigValue(
            "transitclock.avl.playbackSkipIntervalMinutes",
            120,
            "If no data for this amount of minutes skip forward in time.");

    protected static IntegerConfigValue playbackStartDelayMinutes = new IntegerConfigValue(
    ...
    protected List<AvlReport> getBatchOfAvlReportsFromDb() {
        ...
        List<AvlReport> avlReports =
                AvlReport.getAvlReportsFromDb(new Date(start), new Date(end), getPlaybackVehicleId(), "ORDER BY time");
    ...
    public void run() {
        ...
        while (dbReadBeginTime < System.currentTimeMillis()
            ...
            for (AvlReport avlReport : avlReports) {
                ...
                if (playbackRealtime.getValue() == true) {
                    if (last_avl_time > -1) {
                        try {
                            // only sleep if values less than playbackSkipIntervalMinutes minutes.
                            // This is to allow it skip days/hours of missing data.
        ...
        logger.info("Read in AVL in playback mode all the way up to current " + "time so done. Exiting.");
