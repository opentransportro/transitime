...
public class AmigoCloudAvlModule extends AvlModule {
    ...
    private static LongConfigValue userId = new LongConfigValue(
            "transitclock.avl.amigocloud.userId",
            1194L,
            "The ID of the client. Obtained using the amigocloud api token via \"curl"
                    + " https://www.amigocloud.com/api/v1/me?token=API_TOKEN\"");
    ...
    private static class MyAmigoWebsocketListener implements AmigoWebsocketListener {
         ...
         * Restarts the connection after a few seconds. The delay might be important to make sure
         * that don't keep repeatedly reconnecting. Plus the old connection might not be completely
         * closed when onClose() is called since can't tell from the documentation on WebSocket.
         *
         * @param code
         * @param reason
         */
        ...
        public void onClose(int code, String reason) {
            logger.error(
                    "AmigoCloudAvlModule.onClose() called so will restart " + "the connection after 10 seconds...");
            ...
            try {
                ...
                for (int i = 0; i < argsArray.length(); ++i) {
                    ...
                    for (int j = 0; j < dataArray.length(); ++j) {
                        ...
                        String vehicleId =
                                objectId.substring(0, objectId.indexOf('-')).trim();
                        ...
                        AvlReport avlReport =
                                new AvlReport(vehicleId, timestamp, latitude, longitude, speed, heading, "AmigoCloud");
    ...
    public void startNonRealtimeWebsockets() {
        AmigoWebsockets socket =
                new AmigoWebsockets(userId.getValue(), feedUrl.toString(), new MyAmigoWebsocketListener(this));
        socket.connect();
    }
    ...
    public void startRealtimeWebsockets() {
        ...
        do {
            try {
                ...
                AmigoWebsockets socket = new AmigoWebsockets(
                        userId.getValue(),
                        projectId.getValue(),
                        datasetId.getValue(),
                        feedUrl.toString(),
