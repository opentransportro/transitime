...
public class TranslocAvlModule extends PollUrlAvlModule {
    ...
    private static StringConfigValue feedAgencyId = new StringConfigValue(
            "transitclock.avl.transloc.agencyId",
            "255",
            "The number Transloc agency ID obtained using the Transloc " + "API agencies.json command.");

    private static StringConfigValue apiKey = new StringConfigValue(
            "transitclock.avl.transloc.apiKey",
            "tXHlJmqevJmsh4q37xKuv3k2vfZ5p1VbAvPjsnwErq18jMCSmb",
            "The API key for the Transloc API.");

    private static final DateFormat translocTimeFormat = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ssX");
    ...
    protected Collection<AvlReport> processData(InputStream in) throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); ++i) {
                ...
                // Process AVL report
                AvlReport avlReport = new AvlReport(vehicleId, gpsTime.getTime(), lat, lon, speed, heading, "Transloc");
