...
public class GtfsFileProcessor {
    ...
    private void obtainGtfsFiles() throws IllegalArgumentException {
        ...
        if (gtfsDirectoryName != null && (gtfsUrl != null || gtfsZipFileName != null)) {
            throw new IllegalArgumentException("For GtfsFileProcessor both the addOptionGtfsDirName() and "
    ...
    /**
     * Handles command line option for doubles. If the option is not set or cannot be parsed then
    ...
    private static GtfsFileProcessor createGtfsFileProcessor(CommandLine commandLineArgs) {
        ...
        double pathOffsetDistance = getDoubleCommandLineOption("pathOffsetDistance", 0.0, commandLineArgs);
        double maxStopToPathDistance = getDoubleCommandLineOption("maxStopToPathDistance", 60.0, commandLineArgs);
        double maxDistanceBetweenStops = getDoubleCommandLineOption("maxDistanceBetweenStops", 6000.0, commandLineArgs);
        boolean disableSpecialLoopBackToBeginningCase =
        ...
        int defaultWaitTimeAtStopMsec =
                getIntegerCommandLineOption("defaultWaitTimeAtStopMsec", 10 * Time.MS_PER_SEC, commandLineArgs);
        ...
        boolean trimPathBeforeFirstStopOfTrip = commandLineArgs.hasOption("trimPathBeforeFirstStopOfTrip");
    ...
    public static CommandLine processCommandLineOptions(String[] args) {
        ...
        options.addOption(OptionBuilder.withArgName("notes")
                .hasArg()
        ...
        options.addOption(OptionBuilder.hasArg()
                ...
                .withDescription("Local file name where the GTFS zip file is. " + "It will be unzipped and processed.")
        ...
        options.addOption(OptionBuilder.hasArg()
                .withArgName("meters")
                .withDescription("When set then the shapes from shapes.txt are offset to the right"
                        + " by this distance in meters. Useful for when shapes.txt is"
                        + " street centerline data. By offsetting the shapes then the"
                        + " stopPaths for the two directions won't overlap when zoomed"
                        + " in on the map. Can use a negative distance to adjust"
                        + " stopPaths to the left instead of right, which could be"
                        + " useful for countries where one drives on the left side of"
                        + " the road.")
        ...
        options.addOption(
                "storeNewRevs", false, "Stores the config and travel time revs into ActiveRevisions " + "in database.");
        ...
        options.addOption(OptionBuilder.hasArg()
                ...
                .withDescription(
                        "Maximum distance that is expected to have between two consecutives" + " stops. in meters. ")
                .create("maxDistanceBetweenStops"));

        options.addOption(
                "integrationTest", false, "Flag to indicate whether import is being run as part of integration test");
