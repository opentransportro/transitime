...
public class ScheduleStatistics {
     ...
     * standard deviation, min, max, and number of data points. Sets everything in the Stats
     * structure except for the bestValue member.
     *
     ...
     * @param originalGtfsStopTime The original data from stop_times.txt for this trip/stop. Used
     *     for filtering out outliers. Can be null when using test data instead of data from
     *     stop_times.txt.
    ...
    private static Stats getStatisticsForTripStop(
        ...
        if (originalGtfsStopTime != null) {
            originalScheduleTime = arrivalsOrDepartures == ArrivalsOrDepartures.ARRIVALS
    ...
    /**
     * Goes through the Collection of Stats objects passed in and determines the fraction (0.0 -
     * 1.0) that are before the number of standardDeviations specified before the mean.
     *
     ...
     */
    private static double determineFractionEarly(double standardDeviations, Collection<Stats> statistics) {
        int totalTimes = 0;
        int totalEarly = 0;
     ...
     * best arrival/departure value to use into statsResultsByTripStopMap. This is done on a per
     * route basis since it is expected that the distribution of the stats will vary between routes.
     *
     ...
     * @param allowableDifferenceFromMeanSecs Specifies how consistent arrival/departure times from
     *     Db must be in order to be used.
     * @param allowableDifferenceFromOriginalTimeSecs Specifies how consistent arrival/departure
     *     times from Db must be in order to be used.
    ...
    public static void determineStatsForRoute(
        ...
        Map<TripStopKey, Stats> statsForRoute = new HashMap<TripStopKey, Stats>(tripStopKeysForRouteFromDb.size());
        for (TripStopKey tripStopKey : tripStopKeysForRouteFromDb) {
            ...
            Stats statsForTripStop = ScheduleStatistics.getStatisticsForTripStop(
                    timesFromDb,
        ...
        for (TripStopKey tripStopKey : tripStopKeysForRouteFromDb) {
            ...
            if (!Float.isNaN(stats.standardDeviation)) bestValue -= stats.standardDeviation * standardDeviationsToUse;
    ...
    public static void processScheduleAdherence(
        ...
        do {
            ...
            if (nextGtfsStopTime == null || !currentGtfsStopTime.getTripId().equals(nextGtfsStopTime.getTripId())) {
        ...
        double percentageOnTimeOrigSched =
                100.0 * (totalDataPoints - numberEarlyOrigSchedule - numberLateOrigSchedule) / totalDataPoints;
        logger.info(
        ...
        double percentageOnTimeNewSched =
                100.0 * (totalDataPoints - numberEarlyNewSchedule - numberLateNewSchedule) / totalDataPoints;
        logger.info(
