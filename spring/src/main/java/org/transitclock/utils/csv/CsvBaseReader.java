...
public abstract class CsvBaseReader<T> {
    ...
    private void parse() {
        ...
        try {
            ...
            logger.debug("Finished CSV parsing of file {}. Took {} msec.", fileName, Duration.between(timer, Instant.now()));

            int lineNumberWhenLogged = 0;
            timer = Instant.now();
            Instant loggingTimer = Instant.now();
            ...
            while (iterator.hasNext()) {
                ...
                if (record.getRecordNumber() >= lineNumberWhenLogged + LINES_TO_PROCESS_BEFORE_CHECKING_IF_SHOULD_LOG) {
                    ...
                    if (Duration.between(loggingTimer, Instant.now()).toMillis() > SECONDS_ELSAPSED_UNTIL_SHOULD_LOG * Time.MS_PER_SEC) {
                        logger.info("  Processed {} lines. Took {} msec...", lineNumberWhenLogged, Duration.between(timer, Instant.now()));
                        loggingTimer = Instant.now();
                    }
                }
            ...
            logger.info(
                    ...
                    numberRecords,
                    fileName,
                    Duration.between(timer, Instant.now()));
