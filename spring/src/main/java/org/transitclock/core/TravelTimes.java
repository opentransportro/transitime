...
import org.transitclock.core.domain.TravelTimesForStopPath;
...
public class TravelTimes {
     ...
     * the distance is as the crow flies but vehicle will actually be traveling on various roads.
     * And we don't know how fast it can really travel on those roads. But doing the best we can
     * here.
     *
     ...
     * @return
     */
    private static int adjustTravelTimeForWaitStop(int timeOfDaySecs, int travelTimeMsec, Indices indices) {
     ...
     * @param referenceTime For converting seconds into day schedule time to an epoch time. Just
    ...
    public static long scheduledDepartureTime(Indices indices, long referenceTime) {
        ...
        if (scheduleTime == null) {
            logger.error(
                    "Called scheduledDepartureTimePlusWaitTime() for stop " + "that doesn't have a schedule time. {}",
                    indices);
            return -1;
        }
    ...
    /**
     * Returns the time vehicle is expected to take to travel from the spatialMatch to the stop at
     * the end of the current stop path. Does not include the stop time for the stop at the end of
     * the stop path.
     *
    ...
    public int expectedTravelTimeFromBeginningOfStopPathToMatch(SpatialMatch match) {
        ...
        int travelTimeForPartialSegment =
                travelTimesForStopPath.getTravelTimeSegmentMsec(timeTravelInfo.indexOfPartialSegment);
        int travelTimeInPartialSegmentToMatch = (int) (travelTimeForPartialSegment * timeTravelInfo.fractionCompleted);
    ...
    /**
     * Returns travel time for the path specified by the indices parameter. Does not include the
     * stop time.
     *
    ...
    /**
     * Determines expected travel time in msec between the two matches based on the travel times
     * from the database.
     *
    ...
    public int expectedTravelTimeBetweenMatches(
        ...
        if (!specialNoSchedCase) {
            ...
            while (indices.isEarlierStopPathThan(endIndices)) {
                ...
                logger.debug(
                        "For vehicleId={} adding stop path travel time={} " + "msec so travel time now is {} for {}",
                        vehicleId,
                        stopPathTravelTime,
                        travelTimeMsec,
                        indices);
        ...
        // Return the results
        logger.debug("For vehicleId={} returning total travel time={} msec. {}", vehicleId, travelTimeMsec, match2);
        return travelTimeMsec;
    }

    /**
     * Determines expected travel time in msec between the two matches based on the travel times
     * from the database. Same as other travelTimeBetweenMatches() method but uses Date to determine
