...
import org.transitclock.core.domain.Arrival;
import org.transitclock.core.domain.ArrivalDeparture;
import org.transitclock.core.domain.AvlReport;
import org.transitclock.core.domain.AvlReport.AssignmentType;
...
import org.transitclock.core.domain.VectorWithHeading;
...
public class VehicleState {
     ...
     *     assigned.
     * @param assignmentMethod How vehicle was assigned (AVL feed, auto assigner, etc). Set to null
    ...
    /**
     * Sets the block for this VehicleState to null. Also sets assignmentId to null and predictable
     * to false.
     *
     * @param assignmentMethod How vehicle was assigned (AVL feed, auto assigner, etc). Set to null
     *     if vehicle not assigned.
    ...
    /**
     * Determines if vehicle is currently getting assigned and it is getting assigned back to the
     * same block it was assigned to just a while ago. In other words this tells if vehicle might
     * have become unpredictable but then is getting reassigned again. In this kind of situation
     * don't want to for example determine arrivals/departures back to the beginning of the block
     ...
     * @return TemporalMatch for vehicle that is at least minimumAge old, or null if there isn't
     *     such a match.
     */
    ...
    /**
     * To be called when predictable vehicle has no valid spatial/temporal match. Only allowed so
     * many of these before vehicle is made unpredictable.
     */
    ...
    /**
     * Returns true if the last AVL report was successfully matched to the assignment indicating
     * that can generate predictions and arrival/departure times etc.
     *
     ...
     * of the GTFS route_short_name and the GTFS route_long_name so get a full name of something
     * like "38 - Geary".
     *
     ...
     * stop but you can have a wait stop that is not a layover due to the vehicle not being allowed
     * to go away from the stop.
     *
    ...
    /**
     * Returns the next to last temporal match. Returns null if there isn't one. Useful for when
     * need to compare the previous to last match with the last one, such as for determining if
     * vehicle has crossed any stops.
     *
     * @return
     */
    ...
    /**
     * Goes through the AVL history for the vehicle and returns AVL report that is at least
     * minimumAge old. If there isn't a match that old then returns null.
     *
     ...
     * If the avlReport does not have an assignment then false is returned such that the old
     * assignment continues to be used. If a block assignment then simply checks the new block ID
     * from the AVL report to the existing block ID. If using trip assignment in AVL feed then the
     * trip is converted to a block assignment for doing the comparison. When using a route
     * assignment then makes sure the route ID has not changed. For when reassigning a vehicle via
     * the AVL feed.
     *
    ...
    /**
     * Returns true if previously the vehicle had the same assignment but that assignment was
     * recently removed due to a problem where the vehicle shouldn't be assigned to that assignment
     * again. A specific example is that this happens if an exclusive block assignment is grabbed by
     * another vehicle. Even though the original vehicle with the assignment might continue to get
     * that assignment via the AVL feed don't want to reassign it to the problem assignment again.
     *
     * <p>BUT WHAT ABOUT A VEHICLE SIMPLY BECOMING UNPREDICTABLE BECAUSE IT TEMPORARILY WENT OFF
     * ROUTE FOR 3 AVL REPORTS?? IN THAT CASE WANT VEHICLE TO MATCH ASSIGNMENT AGAIN. OR WHAT IF
     * BLOCK SIMPLY ENDED?? SEEMS THAT NEED TO REMEMBER IF VEHICLE WAS UNASSIGNED IN SUCH A WAY THAT
     * SHOULDN'T TAKE THAT ASSIGNMENT AGAIN FOR A WHILE.
     *
     * <p>An old assignment is considered recent if the unassignment happened within the last 2
     * hours.
     *
     * @param avlReport
    ...
    /**
     * Can be the blockId, tripId, or tripShortName depending on the type of assignment received
     * from the AVL feed.
     *
    ...
    /**
     * Records the specified arrival as one that still needs to be stored to the db. This is
     * important because can generate arrivals into the future but need to make sure that the
     * arrival is before the subsequent departure and can't do so until get additional AVL reports.
     *
     * @param arrival
     */
    ...
    /**
     * Returns the current real-time schedule adherence for the vehicle, or null if schedule
     * adherence not currently valid (vehicle is not predictable or running a non-schedule based
     * assignment).
     *
     ...
     *
     * @return Heading of vehicle according to path segment. NaN if not currently matched or there
     *     is no heading for that segment.
     */
    ...
    private float getNextPathHeadingIfAtLayover() {
        ...
        double distanceToLayoverStop = match.getStopPath()
                ...
                .distance(getAvlReport().getLocation());
    ...
    /**
     * Normally uses the heading from getPathHeading(). But if that returns NaN then uses recent
     * valid heading from last AVL report, though that might be NaN as well. This can be better then
     * always using heading from AVL report since that often won't line up with the path and can
     * make vehicles be oriented in noticeably peculiar ways when drawn on an map.
     *
