...
import org.transitclock.core.domain.Match;
...
public class TravelTimesProcessor {
     ...
     * this class. The key is made up of the tripId, the stopPathIndex, and the stopId. The stopId
     * is included so that can see if the historic data stopPathIndex and stopId match for the
     * currently configured trip. This is important for making sure that don't use historic data for
     * a stopPathIndex when stops have been removed from or added to the trip.
     */
    public static class ProcessedDataMapKey extends MapKey {
        ...
        public String toString() {
            return "ProcessedDataMapKey [" + "tripId=" + o1 + ", stopPathIndex=" + o2 + ", stopId=" + o3 + "]";
        }
    }
    ...
    /**
     * The travel time length is the length of the stop path divided into equal segments such that
     * the segments are no longer than maxTravelTimeSegmentLength.
     *
    ...
    /**
     * Gets the Matches that are associated with the stop path specified by arrDep2 (the path
     * leading up to that stop) for the same trip (makes sure it is for the same tripId for the same
     * day of the year). The returned matches will include the departure time from the first stop
     * (arrDep1), in between matches, and the arrival time as the second stop (arrDep2).
     *
     * @param dataFetcher
    ...
    private static List<MatchPoint> getMatchPoints(
        ...
        for (Match match : matchesForStopPath) {
            matchPoints.add(new MatchPoint(
                    match.getDate(), match.getDistanceAlongStopPath(), MatchPoint.MatchPointReason.MATCH));
        }

        // Add the arrival time to the list of data points
        matchPoints.add(
                new MatchPoint(arrDep2.getDate(), arrDep2.getStopPathLength(), MatchPoint.MatchPointReason.ARRIVAL));
    ...
    /**
     * Determines the travel times. If stop path is short enough such that it is only a single
     * travel time segment then returns single travel time set to the difference between the
     * departure time for arrDep1 and the arrival time for arrDep2. For when stop path is long
     * enough such that there are multiple travel time segments, for the particular stop path, as
     * specified by the arrival stop, looks up the associated Matches in order to determine how the
     * vehicle travels along the stop path. Determines when the travel time segment vertices are
     * crossed and uses the vertices, along with the departure time and the arrival times at the
     * ends of the stop path, to determine the travel time for each travel time segment for this
     * particular trip.
     *
     * @param dataFetcher
     * @param arrDep1 The departure stop
     ...
     * departure for the same stop or a departure for one stop and then an arrival for the
     * subsequent stop. If the schedule adherence is off too much (by MAX_SCHED_ADH_SECS) then the
     * data is ignored. If schedule adherence is acceptable then the resulting travel and stop/dwell
     * times are put into the stopTimesMap and travelTimesMap members for further processing.
     *
    ...
    private void processDataBetweenTwoArrivalDepartures(
        ...
        // the stop time.
        if (arrDep1.getStopPathIndex() == arrDep2.getStopPathIndex() && arrDep1.isArrival() && arrDep2.isDeparture()) {
            ...
            if (dwellTimeMsec >= 0) addStopTimeToMap(mapKeyForTravelTimes, dwellTimeMsec);
                ...
                logger.error(
                        "Ignoring negative dwell time={} for stop path " + "at arrival/departures {} and {} (key = {})",
                        dwellTimeMsec,
                        arrDep1,
                        arrDep2,
        ...
        if (arrDep1.getStopPathIndex() - arrDep2.getStopPathIndex() != 1
            ...
            List<Integer> travelTimesForStopPath = determineTravelTimesForStopPath(dataFetcher, arrDep1, arrDep2);
    ...
    /**
     * Process historic data from database for single trip. Puts resulting data into stopTimesMap
     * and travelTimesMap.
     *
     ...
     * etc.
     *
     * <p>Only uses historic travel time data if the number of travel times segments in the data
     * match the number of travel time segments needed for the current trip configuration (which
     * depends on the current path length for the trip). This is important because configuration can
     * change (stops can move or the path between stops can change). Therefore need to make sure
     * that only using data if it applies to the current config (has same number of travel time
     * segments). This way can use as much historic info as possible, yet not try to use data that
     * doesn't pertain.
     *
     ...
     * @param stopPathIndex Used to determine if the historic data for the trip has the proper
    ...
    /**
     * Takes the data from the stopTimesMap and travelTimesMap and creates corresponding travel
     * times. Puts those travel times into the TravelTimeInfoMap that is returned.
     *
    ...
    public TravelTimeInfoMap createTravelTimesFromMaps(Map<String, Trip> tripMap) {
        ...
        for (ProcessedDataMapKey mapKey : combinedKeySet) {
            ...
            if (stopTimesForStopPathForTrip != null) {
                ...
                if (mapKey.getStopPathIndex() != trip.getNumberStopPaths() - 1) {
                    logger.debug(
                            "No stop times for {} even though there are " + "travel times for that map key", mapKey);
                } else {
                    unmatched++;
                }
            }
            ...
            TravelTimeInfo travelTimeInfo = new TravelTimeInfo(
                    trip, mapKey.getStopPathIndex(), averagedStopTime, averageTravelTimes, travelTimeSegLength);
        ...
        logger.info(
                "Processing data (total={} matched={} unmatched={} invalid={}) into a"
                        + " TravelTimeInfoMap took {} msec.",
                setSize,
                matched,
                unmatched,
                invalid,
