...
import org.transitclock.core.domain.ScheduleTime;
import org.transitclock.core.domain.StopPath;
import org.transitclock.core.domain.Trip;
import org.transitclock.core.domain.Vector;
...
public class SpatialMatch {
    ...
    /**
     * For making a copy of the SpatialMatch for the same trip pattern but for a new trip/block.
     * This is useful when doing things by TripPattern, such as spatial matching. Can then make
     * copies for the other trips that use that TripPattern.
     *
    ...
    /**
     * Constructs a new SpatialMatch but at the new indices specified. Useful for create a match
     * that is just before or after a stop, which is useful for determining travel time from stop to
     * the next match.
     *
     * @param toCopy
     * @param newIndices
    ...
    /**
     * Returns whether this SpatialMatch is within the specified distance along the path from the
     * end of the trip.
     *
     * @param distance
    ...
    public boolean atEndOfPathStop() {
        return atStop != null && atStop.getTripIndex() == tripIndex && atStop.getStopPathIndex() == stopPathIndex;
    }

    /**
     * Returns true if the match is to the stop for the previous stop path. This can be important
     * because atStop can also be set to the stop for the current stop path, which is very
     * different. If match is not for at a stop then null is returned.
     *
    ...
    /**
     * For when need to determine arrival time when vehicle matches to a stop. Returns a
     * SpatialMatch that corresponds to the end of the stop path for the stop indicated by the
     * current SpatialMatch. If the match is for stop for the previous stop path (the vehicle is
     * matching to just beyond the stop) the the previous stop path is used. Only to be called when
     * vehicle is matched to a stop. Logs error and returns null if match is not for at a stop.
     *
    ...
    /**
     * Determines the next StopPath after this match that has a schedule time. Useful for
     * determining things such as real-time schedule adherence.
     *
     * @return
     */
    public SpatialMatch getMatchAtNextStopWithScheduleTime() {
        ...
        Indices indicesAtStopWithScheduleTime = new Indices(
                getBlock(), getTripIndex(), stopPathIndex, stopPathWithScheduleTime.getNumberSegments() - 1);
    ...
    /**
     * For when need to determine departure time at a stop. Returns a SpatialMatch that corresponds
     * to the beginning of the next stop path for the stop indicated by the current SpatialMatch.
     * Only to be called when vehicle is matched to a stop. Logs error and returns null if match is
     * not for at a stop.
     *
    ...
    /**
     * Returns a SpatialMatch that corresponds to the current match but will be moved to after a
     * stop if the current match is just before the stop. This way can pass in this modified match
     * to the methods that determine travel time and the layover/stop time for the stop where the
    ...
    /**
     * Returns a SpatialMatch that corresponds to the current match but will be moved to before a
     * stop if the current match is just after the stop. This way can pass in this modified match to
     * the methods that determine travel time and the layover/stop time for the stop where the
     * SpatialMatch is will not be included in the calculations. If the current object is not just
    ...
    public SpatialMatch getMatchBeforeStopIfAtStop() {
        ...
        return new SpatialMatch(
                this, previousPathIndices, previousPathIndices.getSegment().length());
    }
    ...
    public SpatialMatch getMatchAtPreviousStop() {
        ...
        // Return a match that is at the end of the path
        Vector segmentVector = indices.getBlock()
                .getSegmentVector(indices.getTripIndex(), indices.getStopPathIndex(), indices.getSegmentIndex());
     ...
     * and back from the match to see if it is within the allowable distance of the stop. Intended
     * to be called by constructor to set the atStop member variable so that only have to determine
     * if SpatialMatch is at the stop once.
     *
    ...
    public int getScheduledWaitStopTimeSecs() {
        try {
            ...
            logger.error("Tried to get wait stop time for a stop that didn't " + "have one. {} {}", this, e, e);
            return -1;
        }
    }
    ...
    public static int numberStopsBetweenMatches(SpatialMatch match1, SpatialMatch match2) {
        ...
        int totalStopsBetweenMatches = stopIdxInLastTrip - stopIdxInFirstTrip + numIntermediateTripsStops;
    ...
    /**
     * Can be either at stop at beginning of the stop path or at the stop at the end of the stop
     * path.
     *
     * @return VehicleAtStopInfo object containing trip and path index of stop that match is nearby.
     *     Returns null if match is not near a stop.
    ...
    /**
     * Returns the VehicleAtStopInfo if match indicates that vehicle is at the stop at the end of
     * the path.
     *
    ...
    /**
     * Returns true if this match indicates that the vehicle is at the stop indicated by the
